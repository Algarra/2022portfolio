...
const fetchExchangeData = () => {
	return fetch('https://api.exchangerate.host/latest', { next: { revalidate: 300 } }).then(response => response.json())
}
...
const Home = async () => {
	const actualExchanges = await fetchExchangeData()
	const lastWeekExchanges = await fetchLastWeekExchangeData()
