...
export const TableModal: FC<TableModalProps> = ({ showModal, setShowModal }) => {
	const [ibanError, setIbanError] = useState('')
	const [newAccount, setNewAccount] = useState(initialFormData)

	const { setNotification, apiErrorManagement, setAccounts, accounts } = useContext(bankContext)
	...
	const addNewAccount = async (account: accountDetails) => {
		try {
			try {
				...
				setAccounts(result.data.accountsList)
