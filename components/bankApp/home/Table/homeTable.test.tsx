...
test('Render table without navigation', async () => {
  render(
    <globalContext.Provider value={{ ...initialGlobalContext, accounts: accounts.slice(0, 4) }}>
...
test('Closing account', async () => {
  ...
  render(
    <globalContext.Provider value={{ ...initialGlobalContext, accounts, setNotification }}>
      <Table countries={[]} currencies={[]} />
    </globalContext.Provider>,
  );
